...
impl<'a, D: DBAccess> SnapshotWithThreadMode<'a, D> {
    ...
    pub fn raw_iterator(&self) -> DBRawIteratorWithThreadMode<'a, D> {
        let readopts = ReadOptions::default();
    ...
    pub fn raw_iterator_opt(&self, mut readopts: ReadOptions) -> DBRawIteratorWithThreadMode<'a, D> {
        readopts.set_snapshot(self);
    ...
    pub fn get_pinned<K: AsRef<[u8]>>(&self, key: K) -> Result<Option<DBPinnableSlice<'a>>, Error> {
        let readopts = ReadOptions::default();
